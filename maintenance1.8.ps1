...
function findowner{
    ...
    $vmmsoper = Invoke-Command -Session $hvsession -ScriptBlock {param ($pc,$cred) Get-WinEvent -LogName "Microsoft-Windows-Hyper-V-VMMS-Operational" -ComputerName $pc -Credential $cred | where {$_.timecreated -ge (Get-Date).AddDays(-37)} | where {$_.id -eq 27311}} -ArgumentList $chvhost,$sup2admcred
