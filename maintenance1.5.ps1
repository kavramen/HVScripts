...
#first run should be done without event log age limitation. At least 1 VM should be present on the host.
function findowner{
    param(
    ...
    if ($vmsonhost -ne $null){
        ...
        $file | ConvertTo-Csv -NoTypeInformation | add-Content -Path "C:\scripting\vmlist_$chvhost.csv" 
    }
    else
    {
...
#loading notify.csv
    ...
    Try
    {
        ...
        $dat = (get-date -UFormat "%y.%m.%d").ToString()
